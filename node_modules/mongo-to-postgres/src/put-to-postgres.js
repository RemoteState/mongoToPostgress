...
export default async ({ knex, collections, tableName, rows }) => {
  ...
  for (const currentRow of rows) {
    ...
    // map foreign keys
    if (foreignKeys) {
      for (let [fieldNames, collectionName] of Object.entries(foreignKeys)) {
        let [fieldName ,newFieldName] = fieldNames.split(" ")
        ...
        if (!Array.isArray(currentRow[fieldName])) {
          ...
          currentRow[newFieldName || fieldName] = mapedField
            ? currentRow[fieldName] = mapedField.newId
            : currentRow[fieldName] = null;
          if (newFieldName) delete currentRow[fieldName]
